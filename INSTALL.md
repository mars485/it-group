...
В разделе «Настройки сайта» суперпользователь может задать бренд, публичный URL, логотип, контакты, цвета, тему публичного сайта, семейство и размер шрифта, аналитику, отдельный email для заявок, SMTP, Telegram и CRM webhook. Email контактов отображается на сайте, а «Email для заявок» получает уведомления. Если они пустые, используется `CONTACT_EMAIL` из `.env`. Логотип (PNG, JPG или WebP до 2 МБ) хранится в томе медиа. Тема, шрифт и масштаб текста (90, 100, 110 или 120%) применяются сразу после сохранения; посетители не видят переключателя темы. Для тем есть отдельные поля цвета фона. При обновлении существующих установок сохраняется тёмная тема.
