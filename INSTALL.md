...
- исходный код репозитория `mars485/it-group` и доступ GitHub к нему (репозиторий приватный; на сервере используйте SSH-ключ с доступом к репозиторию);
...
Войдите по SSH и клонируйте приватный репозиторий. Пример для SSH-ключа, которому разрешён доступ:

```bash
mkdir -p ~/apps
cd ~/apps
git clone git@github.com:mars485/it-group.git
cd it-group
```

Если GitHub сообщает `Permission denied`, добавьте публичный SSH-ключ сервера в доступ к приватному репозиторию или войдите через GitHub CLI. Не записывайте токен доступа в команду `git clone`: он может сохраниться в истории терминала.
...
В разделе «Настройки сайта» укажите публичные контакты и цвета. Email из этих настроек отображается на сайте и используется для получения заявок. Если он пустой, адрес уведомлений берётся из `CONTACT_EMAIL` в `.env`. Для отправки почты с сервера также заполните SMTP-переменные из следующего шага.

### Шаг 7. Подключить email, CRM и аналитику

Откройте `.env` на сервере и заполните нужные значения:
...
`EMAIL_HOST_PASSWORD` — секрет: не публикуйте `.env` и не отправляйте его в переписке. Параметры SMTP берите у почтового провайдера. Если SMTP не настроен, заявки всё равно сохраняются в админке, но email-уведомления не отправляются.

ID Яндекс Метрики, ссылки Telegram/WhatsApp и контактные данные можно заполнить в панели управления. `CRM_WEBHOOK_URL` пока задаётся в `.env`; перед использованием подключите webhook с подходящей авторизацией в вашей CRM.
...
Ключ сервера должен иметь доступ к приватному репозиторию. Если провайдер не даёт SSH-доступ к GitHub, загрузите исходный код через защищённый механизм деплоя панели хостинга.
...
2. Подключите приватный GitHub-репозиторий или загрузите исходники через поддерживаемый механизм деплоя.
...
| Нет уведомлений о заявках | Проверьте email в «Настройках сайта», `CONTACT_EMAIL`, SMTP-хост, порт, TLS, логин, пароль и логи приложения. Сама заявка должна остаться в разделе «Заявки». |
...
- HTTPS, домен, SMTP и доступ к приватному GitHub-репозиторию настраиваются на стороне владельца сервера/провайдера.
